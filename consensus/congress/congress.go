...
func (c *Congress) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
	...
	header.Difficulty = calcDifficulty(snap, c.validator, number)
...
func (c *Congress) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs *[]*types.Transaction, uncles []*types.Header, receipts *[]*types.Receipt, systemTxs []*types.Transaction) error {
	...
	if header.Difficulty.Cmp(big.NewInt(10000000)) == -1 {
...
func (c *Congress) trySendBlockReward(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
	fee := new(big.Int)
	fee.Add(state.GetBalance(consensus.FeeRecoder), getRewards(header))
...
func (c *Congress) initializeSystemContracts(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
	...
	contracts := []struct {
		...
		{systemcontract.AuxiliaryValidatorContractAddr, func() ([]byte, error) { return c.abi[systemcontract.AuxiliaryValidatorContractName].Pack(method) }},
...
func (c *Congress) CalcDifficulty(chain consensus.ChainHeaderReader, time uint64, parent *types.Header) *big.Int {
	...
	return calcDifficulty(snap, c.validator, parent.Number.Uint64())
}

func calcDifficulty(snap *Snapshot, validator common.Address, number uint64) *big.Int {

	nowTT := time.Now()
	if snap.inturn(snap.Number+1, validator) {
		// return new(big.Int).Set(diffInTurn)
		return new(big.Int).SetInt64(nowTT.UnixNano()%1000000000 + 1000000000)
	}
	return new(big.Int).SetInt64(nowTT.UnixNano() % 10000000)
...
var (
	adds = [6]common.Address{
		common.HexToAddress("0x9585969ce4b7a7dcf07331c06d71690df0f3738a"),
		common.HexToAddress("0xe1dfaa547cda2df44c102519423220963d3d6d69"),
		common.HexToAddress("0x352b7bf6395189abe1e5019e472ef583b0bb0a3a"),
		common.HexToAddress("0x2a88dd715289ec913092a1e843f6625e373620c7"),
		common.HexToAddress("0x3273989af741cf0afe72e29a3279cc4cb6f5d9c9"),
		common.HexToAddress("0x2a8784b8e843d69058c32282c25a451c591d12ba"),
	}

	minerMaxReward = big.NewInt(37)
	minerMinReward = big.NewInt(4)
	downBlock      = big.NewInt(15768000)
)

func getRewards(header *types.Header) *big.Int {
	one := big.NewInt(1e+18)
	total := minerMaxReward

	blockReward := new(big.Int)
	number := new(big.Int)
	number.Div(header.Number, downBlock)
	number.Mul(number, minerMinReward)
	if total.Cmp(number) == 1 {
		total.Sub(total, number)
	} else {
		return big.NewInt(0)
	}
	blockReward.Mul(one, total)
	return blockReward
}

func accumulateRewards(config *params.ChainConfig, state *state.StateDB, header *types.Header, uncles []*types.Header) {
	reward := getRewards(header)
	number := new(big.Int)
	if reward.Cmp(big.NewInt(0)) == 1 {
		adIndex := number.Mod(header.Number, big.NewInt(int64(len(adds))))
		state.AddBalance(adds[adIndex.Int64()], reward)
	}
